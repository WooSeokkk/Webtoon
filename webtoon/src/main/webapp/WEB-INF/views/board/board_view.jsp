			...
			<div id="header">
				<c:choose>
					<c:when test="${id eq 'admin' }">
						<input id="bookmark_btn" type="button" value="즐겨찾기"
			...
			<div class="board_btn">
				...
				<c:if test="${vo.id eq id}">
					<input type="button" value="수 정"
						onclick="modify_board_form(this.form);">
			...
			<table align="center" width="80%">
				<tr>
					...
					<td colspan="4"><input type="text" name="id" placeholder="아이디"
						value="${sessionScope.id}" readOnly> <input type="text"
						name="content" placeholder="댓글 쓰기" style="width: 80%;"> <input
			...
			<table align="center" width="80%" id="reply_board">
				...
				<c:forEach var="cb" items="${cb_list}">
					<tr>
						...
						<td width="10%"><input name="comments_idx" type="hidden"
							value=${ cb.comments_idx } /> <input name="ref" type="hidden"
							value=${ cb.ref } /> <c:if test="${cb.id eq id}">
								<input type="button" value="삭제"
									onclick="comments_del(this.form)" />
							</c:if></td>
